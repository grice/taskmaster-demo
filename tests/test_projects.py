...
from tests.conftest import make_project, make_task, make_milestone, W


class TestProjectList:
    def test_empty_list(self, client):
        r = client.get(W + '/projects')
    ...
    def test_status_filter_active(self, client, db):
        ...
        r = client.get(W + '/projects?status=active')
...
class TestProjectDetail:
    def test_shows_project(self, client, db):
        ...
        r = client.get(f'{W}/projects/{p.id}')
        assert r.status_code == 200
    ...
    def test_404_for_missing(self, client):
        r = client.get(W + '/projects/99999')
        assert r.status_code == 404

    def test_gantt_data_json(self, client, db):
        p = make_project()
        make_task(p, 'Gantt Task')
        r = client.get(f'{W}/projects/{p.id}/gantt-data')
        assert r.status_code == 200
        data = r.get_json()
        assert len(data) == 1
...
class TestDeleteProject:
    def test_get_shows_confirmation(self, client, db):
        ...
        r = client.get(f'{W}/projects/{p.id}/delete')
        assert r.status_code == 200
    ...
    def test_post_deletes_project(self, client, db):
        ...
        pid = p.id
        r = client.post(f'{W}/projects/{pid}/delete', follow_redirects=True)
        assert r.status_code == 200
        assert Project.query.get(pid) is None

    def test_post_redirects_to_list(self, client, db):
        p = make_project()
        r = client.post(f'{W}/projects/{p.id}/delete')
        assert r.status_code == 302
        assert '/projects' in r.headers['Location']
...
class TestExcelExport:
    def test_returns_xlsx(self, client, db):
        ...
        r = client.get(f'{W}/projects/{p.id}/export/excel')
        assert r.status_code == 200
    ...
    def test_404_for_missing_project(self, client):
        r = client.get(W + '/projects/99999/export/excel')
        assert r.status_code == 404
    ...
    def test_deletes_cascade_to_tasks(self, client, db):
        ...
        t = make_task(p)
        tid = t.id
        client.post(f'{W}/projects/{p.id}/delete')
