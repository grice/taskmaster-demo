...
from tests.conftest import make_team, make_person, make_project, make_task, W


class TestPeopleList:
    def test_empty_list(self, client):
        r = client.get(W + '/people')
...
class TestPersonDetail:
    ...
    def test_shows_assigned_tasks(self, client, db):
        ...
        db.session.commit()
        r = client.get(f'{W}/people/{person.id}')
...
class TestEditPerson:
    def test_get_edit_form(self, client, db):
        p = make_person('Grace')
        r = client.get(f'{W}/people/{p.id}/edit')
        assert r.status_code == 200
        assert b'Grace' in r.data

    def test_update_person(self, client, db):
        p = make_person('Hank')
        r = client.post(f'{W}/people/{p.id}/edit', data={
            'name': 'Henry',
...
class TestDeletePerson:
    def test_get_shows_confirmation(self, client, db):
        p = make_person('Ivan')
        r = client.get(f'{W}/people/{p.id}/delete')
        assert r.status_code == 200
    ...
    def test_post_deletes_person(self, client, db):
        p = make_person('Jack')
        pid = p.id
        r = client.post(f'{W}/people/{pid}/delete', follow_redirects=True)
        assert r.status_code == 200
...
class TestEditTeam:
    def test_get_edit_form(self, client, db):
        ...
        r = client.get(f'{W}/teams/{t.id}/edit')
        assert r.status_code == 200
        assert b'Old Team' in r.data

    def test_update_team(self, client, db):
        t = make_team('Rename Me')
        r = client.post(f'{W}/teams/{t.id}/edit', data={'name': 'Renamed'},
                        follow_redirects=True)
        assert r.status_code == 200
...
class TestDeleteTeam:
    def test_get_shows_confirmation(self, client, db):
        ...
        r = client.get(f'{W}/teams/{t.id}/delete')
        assert r.status_code == 200
    ...
    def test_post_deletes_team(self, client, db):
        ...
        tid = t.id
        r = client.post(f'{W}/teams/{tid}/delete', follow_redirects=True)
        assert r.status_code == 200
